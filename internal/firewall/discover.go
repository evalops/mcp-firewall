...
func Discover(ctx context.Context, cfg DiscoverConfig) (Policy, error) {
	...
	schemes := uniqueSchemes(resources)
	note := fmt.Sprintf("Generated by mcp-firewall discover: %d tools, %d resources (%s), %d prompts.", len(tools), len(resources), strings.Join(schemes, ", "), len(prompts))
